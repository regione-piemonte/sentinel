...
def get_download_days_range(days_shift):
    current_datetime = datetime.datetime.now()
    days_before_datetime = current_datetime - timedelta(days=days_shift)
    return days_before_datetime, current_datetime
...
# get the two extreme for the datatime range
from_datetime, to_datetime = get_download_days_range(days_before) # generate the string to put inside the url for for the API
date_string_for_url = create_date_range_string_for_url(from_datetime, to_datetime) #generate the query with time and polygon setted into the config file
sentinel_query = insert_query_date(generic_sentinel_query, date_string_for_url)   
query =  insert_query_polygon(sentinel_query, polygon_data_to_download)     
if len(sys.argv) > 1: #overwrite the query automatically created with the one passed on the command line in case one is passed
    query = sys.argv[1] 
  
url = urlSearchApi + query # create the complete new query
log_msg("-----------------------------------")
log_msg("Query:\n" + query) 
log_msg("-----------------------------------")   
try:
    # TODO: usare proxy per chiamate
    #resp = requests.get(url, auth=(CSI_user, CSI_password),proxies={'http': 'http://proxy-srv.csi.it:3128', 'https': 'https://proxy-srv.csi.it:3128'}) # call the api and get the returned xml
...
for child in feed:
    if child.tag == nsAtom + 'entry':
        ...
        nEntry = nEntry + 1 # increment the number of the entry
